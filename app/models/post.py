# -*- coding: utf-8 -*-
from anaconda_navigator.data.pretty_json import content
from win32con import POSTSCRIPT_DATA
posttags = db.Table('posttags', db.Column('tags_id',  db.Integer, db.ForeignKey("tags.id")), db.Column("post_id", db.Integer, db.ForeignKey("post.id")))
...
calss Post(db.Model):
    ...
    category = db.relationship("Category", foreign_keys = [category_id], backref = db.backref('posts', lazy = 'dynamic'))
    tags = db.relationship('Tag', secondaty = posttags, backref = db.backref('posts', lazy = 'dynamic'))
...
class Category(db.Model):
    ...
    parent_category = db.relationship('Category', uselist = False, remote_side = [id], backref = db.backref('son_categirues', uselist = True))
    ...
    def __repr__(self):
        ...
        return '<Category %s Parent %s Son %s>' % (self.name, name, [cg.name for cg in self.son_categories]) 
    ...
    def posts_count(self, query = None):
        if query:
            ...
            for category in self.son_categories:
                sum_posts_count = sun_posts_count + query.filter(Post.category == categoty).count()
        else:
            count = self.posts.count()
            sum_posts_count = 0
            for category in self.son_categories:
                sum_posts_count = sum_posts_count + category.post_count()
