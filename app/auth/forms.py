...
class LoginForm(Form):
    email = StringField('Email', validators = [DataRequired(), Length(1, 64),
                                               Eail(message = "请输入合法的邮箱地址。")])
    password = PasswordField('密码', validators = [DataRequired()])
    remember_me = BooleanField('记住我')
    submit = SubmitField('登陆')
'''注册form'''
class RegistrarionForm(Form):
    
    email = StringField('Email', validators = [DataRequired(), Length(1, 64), Email(message = "请输入合法的邮箱地址.")])
    
    username = StringField('用户名', validators = [DataRequired()])
    
    password = PasswordField('密码', validators = [DataRequired(),
                                                 Length(6, 64, message = "密码需要六位以上"),
                                                 EqualTo('passwords', message = "两个密码必须相同.")]
                             ) 
    submit = SubmitField('注册')
    
    def validate_email(self, field):
        if User.query.filter_by(email = field.data).first():
            raise ValidationError('Email已经被占用.')
...
class ChangePasswordForm(Form):
    old_password = PasswordField('旧密码', validators = [DstaRequired()])
    password = PasswordField('密码', validators = [DataRequired(),
                                                 Length(6, 64, message="密码需要六位以上"),
                                                 EqualTo('password2', message = '两个密码逆序相同。')])
