...
@admin_manager.route('/edit/profile/<int:user_id>', method = ['GET', 'POST'])
@login_required
@admin_required
def edit_profile_admin(user_id = None):
    if user_id:
        use_edited = User.query.get_or_404(user_id)
        ...
        if form.validate_on_submit():
            ...
            new_user.save()
            flash('用户创建成功')
            return redirect(url_for('admin manager.users'))
...
def moderate():
    ...
    pagination = Comment.query.order_by(Comment.timetamp.desc().paginate(
...
def users():
    page = request.args.get('page', 1, type = int)
    pagination = User.query.order_by(User.last_seen.desc().paginate(page, per_page = current_app.config['FOLLOWERS_PER_PAGE'],
                                                                    error_out = False))
    users_list = pagingnation.items 
    return renser_template('admin_manager/users.html', pagination = pagination, users = users_list)
...
@login_requried    
@permission_requried(Permission.MODERATE_COMMENTS)
def categories(category_id = None):
    ...
    categoies_list = pagination.items
    if category_id:
        category_id = Category.query.filter_by(id = category_id).first()
        form = CategoryForm()
        ...
        form.parent.choices = [(category.parent_id, Categories.query.filter_by(id = category.parent_id).first().name)]
    else:
        form = CategoriesFrom()
        if request.method == 'POST' and form.validate():
            new_category = Category(form.name.data, Category.query.filter_by(id = form.parent.data))
            new_category.save()
            flash("已经添加")
            return redirect(url_for('admin_manager.categories', page = page, form = form))
    return render_template('admin_manager/categories.html', form = form, pagination = pagination, categories = categories_list)
